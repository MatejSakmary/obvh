...
static void render_reticle(WuhooRGBA *pixels, i32 width, i32 height, ivec2 at, i32 strech)
{
    i32 p = strech;
...
static void export_image_f32(u8 *pixels, u32 width, u32 height, const char *filename)
{
...
static void export_image(u8 *pixels, u32 width, u32 height, const char *filename)
{
    kr_texture texture = {0};
    texture.dims = {width, height, 0};
    texture.data = pixels;
...
static bool render_gl(kr_ctx *ctx)
{
#ifdef KR_USE_OPENGL
...
int main(int argc, char *argv[])
{
    ...
    if (param.values != kr_null)
    {
        use_collapse = kr_true;
    }
    ...
    for (kr_size module_index = 0; module_index < module_count; module_index++)
    {
        ...
        if (kr_success != result)
            kr_log("Failed to load integrator '%s'\n", module_names[module_index]);
        else
            kr_log("Loaded integrator '%s'\n", module_names[module_index]);
    }

    ctx->integrator = &ctx->integrators[module_index];
    if (kr_null == ctx->integrator)
        return -1;
    ...
    if (KR_EQUALS_LITERAL(benchmark_type, "ads build") ||
        KR_EQUALS_LITERAL(benchmark_type, "ads sah"))
    {
        ctx->scene.benchmark_file = benchmark_file;
    }
